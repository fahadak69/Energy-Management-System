 ...
 */
#define IIC_SLAVE_ADDR_W		0xAA //Write Address for TI I2C Battery Guage
#define IIC_SLAVE_ADDR_R		0xAB //Read Address for TI I2C Battery Guage
...
int IicPsMasterIntrExample(u16 DeviceId)
{
	 ...
	 * verified.
	 */
	SendBuffer[Index] = 0x29; //Address of I2C slave SERIAL NUMBER register for TI Battery Guage
	RecvBuffer[Index] = 0;

	for(Index = 0; Index < 1; Index++) { ////////////////// running for 1byte only
		 ...
		 * correctly.
		 */
		while (!SendComplete) { /////////////////////////////////////////// instead of this while loop, i think its better to use sem_post in signhandler and sem_wait here
		...
		for(tmp = 0; tmp < BufferSizes[Index]; tmp ++) {
			 ...
			 * output.
			 */
			printf ("Recieved Byte %d is equal to %d",tmp,RecvBuffer[tmp]);
...
void Handler(void *CallBackRef, u32 Event)
{
	...
	if (0 != (Event & XIICPS_EVENT_COMPLETE_RECV)){
		printf ("In Receive Handler\n");/////////////////////////FA
		RecvComplete = TRUE;
	} else if (0 != (Event & XIICPS_EVENT_COMPLETE_SEND)) {
		printf ("In Send Handler\n");//////////////////////////////////FA
		SendComplete = TRUE;
