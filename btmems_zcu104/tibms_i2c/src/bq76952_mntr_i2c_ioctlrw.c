...
//This code is based upon code in TI application note slua801 (section A.3 page 16) with register numbers modified for battery monitor bq76952, "guage" in the function names replaced with "monitor"
//and system calls for read and write are replaced using ioctl I2C_RDWR TO allow read write in same transaction (i.e. repeated start)
...
#define SLAVE_ADDR7BIT 0x8 // 7bit slave address
...
u_int8_t slv_reg[] = {nRegister};

struct i2c_msg messages[] = {
  { SLAVE_ADDR7BIT, 0, sizeof(slv_reg), slv_reg },
  { SLAVE_ADDR7BIT, I2C_M_RD, nLength, pData }
};
struct i2c_rdwr_ioctl_data ioctl_rw_data = { messages, 2 };

n = ioctl(pI2C->nI2C, I2C_RDWR, &ioctl_rw_data);
//printf ("IOCTL Read Back Bytes %d\n",n);
